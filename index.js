          ...
          <tr>
            <td style="background-color:#0f6b5c; padding:28px 32px;">
              <table role="presentation" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="width:32px; height:32px; background-color:#ffffff; border-radius:9px; text-align:center; font-size:18px; line-height:32px; font-family:Arial,sans-serif; font-weight:700; color:#0f6b5c;">E</td>
...
app.post('/api/auth/verify-otp', async (req, res) => {
  try {
    ...
    if (user.referred_by && !user.referral_bonus_paid) {
      ...
      await logActivity(user.referred_by, 'gift', 'Referral bonus earned', `A friend joined using your code · +${bonus} coins`);
      await logActivity(user.id, 'gift', 'Welcome bonus', `Referral signup bonus · +${bonus} coins`);
    }
...
async function expirePremiumIfNeeded(userId) {
  ...
  if (rows.length) await logActivity(userId, 'hourglass', 'Premium expired', 'Your Premium subscription has ended — renew to keep 2x rewards.');
}
...
app.get('/api/tasks/postback', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE users SET wallet_coins = wallet_coins + $1, total_earned = total_earned + $1 WHERE id = $2',
      [coinsAwarded, userId]);
    await logActivity(userId, 'target', 'Task completed', `+${coinsAwarded} coins`);
...
// ---------- ADGATE MEDIA (third offer source — custom-built UI, not their embedded wall) ----------
let adgateCache = { offers: [], fetchedAt: 0 };
async function getAdgateOffers() {
  const CACHE_MS = 10 * 60 * 1000; // AdGate recommends refreshing at most every 10 minutes, from the backend
  if (Date.now() - adgateCache.fetchedAt < CACHE_MS && adgateCache.offers.length) return adgateCache.offers;
  const { ADGATE_AFF_ID, ADGATE_API_KEY, ADGATE_WALL_CODE } = process.env;
  if (!ADGATE_AFF_ID || !ADGATE_API_KEY || !ADGATE_WALL_CODE) return [];
  const url = `https://api.adgatemedia.com/v3/offers?aff=${ADGATE_AFF_ID}&api_key=${ADGATE_API_KEY}&wall_code=${ADGATE_WALL_CODE}&countries=IN&orderby=payout&take=60`;
  const resp = await fetch(url);
  const json = await resp.json();
  const raw = Array.isArray(json) ? (json[0]?.data || []) : (json?.data || []);
  adgateCache = { offers: raw, fetchedAt: Date.now() };
  return raw;
}

app.get('/api/adgate/offers', requireAuth, async (req, res) => {
  try {
    const raw = await getAdgateOffers();
    if (!raw.length) return res.json({ offers: [] });
    const offers = raw.map(o => {
      const bestEvent = (o.events || []).reduce((max, e) => (e.payout > (max?.payout || 0) ? e : max), null);
      return {
        id: o.id,
        title: o.anchor,
        description: o.description,
        requirements: o.requirements,
        icon: o.creatives?.icon || null,
        payoutUsd: bestEvent ? bestEvent.payout : 0,
        eventLabel: bestEvent ? bestEvent.name : 'Complete',
        clickUrl: `${o.click_url}${o.click_url.includes('?') ? '&' : '?'}s1=${req.userId}`,
      };
    }).filter(o => o.payoutUsd > 0).sort((a, b) => b.payoutUsd - a.payoutUsd);
    res.json({ offers });
  } catch (err) {
    console.error('AdGate offers error:', err);
    res.status(503).json({ error: 'Offers are not available right now.' });
  }
});

// AdGate calls this URL automatically when a user completes an offer. Configure it as your Postback URL
// in the AdGate dashboard (include {state} so reversals are reported too):
//   https://your-backend-url/api/adgate/postback?conversion_id={conversion_id}&user_id={s1}&usd_value={payout}&offer_title={vc_title}&state={state}
app.get('/api/adgate/postback', async (req, res) => {
  try {
    const { conversion_id, user_id, usd_value, offer_title, state } = req.query;
    if (!conversion_id || !user_id) return res.status(400).send('Missing parameters');
    if (state && state !== 'approved') return res.send('OK (not approved, no coins credited)');

    const userId = parseInt(user_id, 10);
    const payoutUsd = parseFloat(usd_value);
    if (!userId || isNaN(payoutUsd)) return res.status(400).send('Invalid parameters');

    const existing = await pool.query('SELECT id FROM offer_completions WHERE network_trans_id = $1', [conversion_id]);
    if (existing.rows.length > 0) return res.send('OK (already processed)');

    const cfg = await getConfig();
    const amountInr = payoutUsd * cfg.usd_to_inr_rate;
    ...
    await pool.query('INSERT INTO offer_completions (user_id, network_trans_id, offer_id, payout_usd, coins_awarded) VALUES ($1, $2, $3, $4, $5)',
      [userId, conversion_id, offer_title || null, payoutUsd, coinsAwarded]);
    await pool.query('UPDATE users SET wallet_coins = wallet_coins + $1, total_earned = total_earned + $1 WHERE id = $2',
      [coinsAwarded, userId]);
    await logActivity(userId, 'target', 'Offer completed', `+${coinsAwarded} coins`);
...
app.get('/api/surveys/postback', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE users SET wallet_coins = wallet_coins + $1, total_earned = total_earned + $1 WHERE id = $2',
      [coinsAwarded, userId]);
    await logActivity(userId, 'clipboard', 'Survey completed', `+${coinsAwarded} coins`);
...
app.post('/api/earn/checkin', requireAuth, async (req, res) => {
  try {
    ...
    await logActivity(u.id, 'calendar', 'Daily check-in bonus', `Day ${newStreak} streak · +${finalCoins} coins`);
...
app.post('/api/earn/spin', requireAuth, async (req, res) => {
  try {
    ...
    const finalCoins = await awardCoins(u.id, coins);
    await logActivity(u.id, 'wheel', 'Spin & Win reward', `+${finalCoins} coins`);
...
app.post('/api/earn/scratch', requireAuth, async (req, res) => {
  try {
    ...
    const finalCoins = await awardCoins(u.id, coins);
    await logActivity(u.id, 'ticket', 'Scratch card reward', `+${finalCoins} coins`);
    res.json({ coinsAwarded: finalCoins });
...
app.post('/api/earn/complete-profile', requireAuth, async (req, res) => {
  try {
    ...
    const finalCoins = await awardCoins(u.id, coins);
    await logActivity(u.id, 'pencil', 'Profile completed', `+${finalCoins} coins`);
    res.json({ coinsAwarded: finalCoins });
...
app.post('/api/earn/claim-badge', requireAuth, async (req, res) => {
  try {
    ...
    await logActivity(u.id, 'badge', `${milestone.key} badge unlocked`, `+${finalCoins} coins`);
    res.json({ coinsAwarded: finalCoins });
...
app.post('/api/admin/premium-requests/:id/approve', requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity(reqRow.user_id, 'crown', 'Premium activated', '2x rewards unlocked for 30 days');
    res.json({ success: true });
  } catch (err) {
    console.error(err);
...
app.post('/api/admin/withdrawals/:id/mark-paid', requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity(w.user_id, 'check', 'Withdrawal approved', `₹${w.amount_inr} has been sent`);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
...
async function maybeFinish(tournament) {
  ...
  if (winner) {
    ...
    await logActivity(winner.user_id, 'trophy', 'Tournament won!', `+${finished.prize_coins} coins`);
  }
  return finished;
}
...
app.post('/api/tournament/:id/leave', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM tournaments WHERE id = $1', [req.params.id]);
    const tournament = rows[0];
    if (!tournament) return res.json({ success: true }); // already gone, nothing to do
    if (tournament.status !== 'waiting') return res.json({ success: true }); // match already started/finished — leaving doesn't change anything
    await pool.query('DELETE FROM tournament_players WHERE tournament_id = $1 AND user_id = $2', [req.params.id, req.userId]);
    const { rows: cRows } = await pool.query('SELECT COUNT(*)::int AS c FROM tournament_players WHERE tournament_id = $1', [req.params.id]);
    if (cRows[0].c === 0) await pool.query('DELETE FROM tournaments WHERE id = $1', [req.params.id]); // clean up empty room
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Could not leave tournament.' });
  }
});
