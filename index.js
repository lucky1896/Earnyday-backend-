...
app.get('/api/wallet/me', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, name, email, is_premium, wallet_coins, total_earned, ads_watched_today, upi_id FROM users WHERE id = $1',
      [req.userId]
    );
    const user = rows[0];
    if (!user) return res.status(404).json({ error: 'User not found' });
    const cfg = await getConfig();
    res.json({ ...user, walletInr: +(user.wallet_coins * cfg.coin_to_inr).toFixed(2), minWithdrawInr: +cfg.min_withdraw_inr, premiumPriceInr: +cfg.premium_price_inr });
  } catch (err) {
    console.error(err);
...
app.post('/api/admin/config', requireAdmin, async (req, res) => {
  try {
    const { coin_per_ad, coin_to_inr, min_withdraw_inr, daily_free_limit, user_share_percent, cpa_user_share_percent, usd_to_inr_rate, referral_bonus_coins, premium_price_inr } = req.body;
    await pool.query(
      `UPDATE config SET
      ...
      WHERE id = 1`,
      [coin_per_ad, coin_to_inr, min_withdraw_inr, daily_free_limit, user_share_percent, cpa_user_share_percent, usd_to_inr_rate, referral_bonus_coins, premium_price_inr]
    );
    res.json(await getConfig());
  } catch (err) {
    console.error(err);
