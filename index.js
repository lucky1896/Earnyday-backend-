...
app.post('/api/admin/config', requireAdmin, async (req, res) => {
  try {
    const { coin_per_ad, coin_to_inr, min_withdraw_inr, daily_free_limit, user_share_percent, cpa_user_share_percent, usd_to_inr_rate, referral_bonus_coins, premium_price_inr, admin_upi_id } = req.body;
    await pool.query(
      `UPDATE config SET
      ...
      WHERE id = 1`,
      [coin_per_ad, coin_to_inr, min_withdraw_inr, daily_free_limit, user_share_percent, cpa_user_share_percent, usd_to_inr_rate, referral_bonus_coins, premium_price_inr, admin_upi_id]
    );
    res.json(await getConfig());
  } catch (err) {
    console.error(err);
